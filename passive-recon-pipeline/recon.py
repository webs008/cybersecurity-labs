...
enumeration, WHOIS lookup, VirusTotal reputation check, theHarvester
OSINT gathering, then generates a consolidated markdown report.
"""
...
from modules.theharvester_lookup import gather_osint
...
def main():
    ...
    if ssl_result["error"]:
        ...
        print(f"[+] Issued by: {ssl_result['issued_by']}, expires {ssl_result['expires']}")
    ...
    if dns_result["error"]:
        ...
        print(f"[+] SPF: {dns_result['spf_found']}, DMARC: {dns_result['dmarc_found']}")
    ...
    # --- theHarvester ---
    print(f"[*] Running theHarvester for {args.domain}...")
    harvester_result = gather_osint(args.domain)
    if harvester_result["error"]:
        print(f"[!] Error: {harvester_result['error']}")
    else:
        print(f"[+] Emails found: {len(harvester_result['emails'])}")
        print(f"[+] Hosts found: {len(harvester_result['hosts'])}")
        print(f"[+] ASNs found: {len(harvester_result['asns'])}")
        print(f"[+] Interesting URLs found: {len(harvester_result['interesting_urls'])}")
